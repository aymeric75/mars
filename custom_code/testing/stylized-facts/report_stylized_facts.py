...
def get_minute_info(trade_infos: list[TradeInfo], start_lob: LobSnapshot) -> list[MinuteInfo]:
    ...
    # # --- hygiene for sparse / no-trade minutes ---
    # for c in ["price_last", "price_mean"]:
    #     minutes_data.loc[minutes_data[c] == -1, c] = np.nan

    # # Use mid-price as fallback when there was no trade
    # minutes_data["price_last"] = minutes_data["price_last"].fillna(minutes_data["price_mid_last"])
    # minutes_data["price_mean"] = minutes_data["price_mean"].fillna(minutes_data["price_mid"])
...
def get_rollout_info(path: Path) -> RolloutInfo | None:
    ...
    Args:
        path: Path to the file containing trade information , a list of two tuples
...
def plot_return_autocorrelation(rollout_infos: list[RolloutInfo], output_dir: Path, price_type: PriceType) -> None:
    ...
    fig_path = output_dir / f"cont1-{price_type}-absence-of-auto-correlations.png"
