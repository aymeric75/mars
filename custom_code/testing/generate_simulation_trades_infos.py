    ...
    *,
    ...
    for _ in range(int(max_gen_orders)):
        ...
        if pred.order_type == "C":
            ...
            o = LimitOrder(
                time=t, symbol=symbol, tag="sim",
                type="C", price=target_price, volume=cancel_vol,
                agent_id=-1, order_id=-1,
                cancel_type=cancel_side, cancel_id=int(target.order_id),
            )
...
order_model = load_order_model(
    ckpt_path="/scratch/project_2012747/mars_runs/order_model/tb_seq/31991415/step=step=3000-val=val_loss=2.1834.ckpt",
    device="cuda"
)
